...
- (void)configureAvatarImageView:(UIImageView *)imageView forMessageType:(JSBubbleMessageType)type
{
    CGFloat avatarX = 1.5f;
    if (type == JSBubbleMessageTypeOutgoing) {
        avatarX = (self.contentView.frame.size.width - kJSAvatarImageSize);
    }
    
    CGFloat avatarY = 20;//self.contentView.frame.size.height - kJSAvatarImageSize;
//    if (_subtitleLabel) {
//        avatarY -= kJSSubtitleLabelHeight;
//    }
    
    imageView.frame = CGRectMake(avatarX, avatarY, kJSAvatarImageSize - 2, kJSAvatarImageSize - 2);
...
- (void)layoutSubviews
{
    ...
    if (self.subtitleLabel) {
        self.subtitleLabel.frame = CGRectMake(kJSLabelPadding, kJSAvatarImageSize + 20,
//                                              self.contentView.frame.size.height - kav kJSSubtitleLabelHeight,
                                              self.contentView.frame.size.width - (kJSLabelPadding * 2.0f),
