...
@interface LCMultiSelectController () <UITableViewDataSource,UITableViewDelegate,MLLetterIndexNavigationViewDelegate,LCMultiSelectedPanelDelegate,UISearchBarDelegate,UISearchDisplayDelegate>
...
@property (nonatomic, strong) LCMultiSelectedPanel *selectedPanel;
...
@implementation LCMultiSelectController
...
- (void)viewDidLoad
{
    ...
    for (LCMultiSelectItem *item in self.items) {
    ...
    for (NSString *key in [self.dict allKeys]) {
        if (((NSArray*)self.dict[key]).count<=0) {
            ...
            self.dict[key] = [((NSArray*)self.dict[key]) sortedArrayUsingComparator:^NSComparisonResult(id obj1, id obj2) {
                return [((LCMultiSelectItem *)obj1).name localizedCompare:((LCMultiSelectItem *)obj2).name];
            }];
        }
    }
    ...
    for (NSUInteger section=0; section<self.keys.count; section++) {
        for (NSUInteger row=0; row<((NSArray*)self.dict[self.keys[section]]).count; row++) {
            LCMultiSelectItem *item = self.dict[self.keys[section]][row];
...
- (LCMultiSelectedPanel *)selectedPanel
{
    if (!_selectedPanel) {
        _selectedPanel = [[LCMultiSelectedPanel alloc] initWithFrame:CGRectMake(0, 0, self.view.frame.size.width, 44.0f)];
...
#define kSelectPanelHeight 44.0f
    self.tableView.frame = CGRectMake(0, 0, CGRectGetWidth(self.view.frame), CGRectGetHeight(self.view.frame)-kSelectPanelHeight);
    
    CGFloat topY = 0.0f;
    //导航View的位置
    if (IOS_VERSION>=7.0) { //简单处理下适配吧
        topY += 20.0f;
        topY += (CGRectGetHeight(self.view.frame)>CGRectGetWidth(self.view.frame))?44.0f:32.0f;
    }
    self.letterIndexView.frame = CGRectMake(CGRectGetMaxX(self.tableView.frame)-20.0f, topY, 20.0f, CGRectGetHeight(self.view.frame)-kSelectPanelHeight-topY);
    
    self.selectedPanel.frame = CGRectMake(0, CGRectGetMaxY(self.tableView.frame), CGRectGetWidth(self.view.frame), kSelectPanelHeight);
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if ([tableView isEqual:self.tableView]) {
        ...
        LCMultiSelectCell *cell = [tableView dequeueReusableCellWithIdentifier:CellIdentifier];
        if (!cell) {
            cell = [[LCMultiSelectCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:CellIdentifier];
            cell.selectionStyle = UITableViewCellSelectionStyleGray;
        }
        
        LCMultiSelectItem *item = ((NSArray*)self.dict[self.keys[indexPath.section]])[indexPath.row];
        
        [cell.cellImageView sd_setImageWithURL:item.imageURL placeholderImage:[UIImage imageNamed:@"head_default"]];
        cell.label.text = item.name;
        if (item.disabled) {
            cell.selectState = LCMultiSelectStateDisabled;
        }else{
            cell.selectState = item.selected?LCMultiSelectStateSelected:LCMultiSelectStateNoSelected;
        }
        
        return cell;
    }
    
    static NSString *CellIdentifier = @"MultiSelectSearchResultCell";
    LCMultiSelectSearchResultCell *cell = [tableView dequeueReusableCellWithIdentifier:CellIdentifier];
    if (!cell) {
        cell = [[LCMultiSelectSearchResultCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:CellIdentifier];
        cell.selectionStyle = UITableViewCellSelectionStyleGray;
    }
    
    LCMultiSelectItem *item = self.searchResult[indexPath.row];
    [cell.cellImageView sd_setImageWithURL:item.imageURL placeholderImage:[UIImage imageNamed:@"head_default"]];
...
{
    if ([tableView isEqual:self.tableView]) {
        UIView *customHeaderView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, CGRectGetWidth(self.view.frame), kDefaultSectionHeaderHeight)];
        customHeaderView.backgroundColor = [UIColor colorWithRed:0.926 green:0.920 blue:0.956 alpha:1.000];
        UILabel *headerLabel = [[UILabel alloc]initWithFrame:CGRectMake(15.0f, 0, CGRectGetWidth(customHeaderView.frame)-15.0f, kDefaultSectionHeaderHeight)];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    for (NSUInteger section=0; section<self.keys.count; section++) {
        for (NSUInteger row=0; row<((NSArray*)self.dict[self.keys[section]]).count; row++) {
            LCMultiSelectItem *aItem = self.dict[self.keys[section]][row];
...
- (void)didConfirmWithMultiSelectedPanel:(LCMultiSelectedPanel*)multiSelectedPanel {
    NSArray *items = multiSelectedPanel.selectedItems;
    [self dismissViewControllerAnimated:YES completion:^{
        if ([_delegate respondsToSelector:@selector(multiSelectController:didSelectItems:)]) {
            [_delegate multiSelectController:self didSelectItems:items];
        }
    }];
}
