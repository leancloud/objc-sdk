...
-(void)demoGetFileData{
    //获取要保存的数据
    
    //用数据创建文件
//    AVFile *file=[AVFile fileWithURL:@"http://ikbear.qiniudn.com/iphone.jpg"];
    AVFile *file=[AVFile fileWithURL:@"http://paas-files.qiniudn.com/O3gIdJtdK85z1dslslRK2wLyCQluFsSbbAJKpPXs.dat"];
    
    //保存文件
//    NSData *data = [file getData];
    [file getDataInBackgroundWithBlock:^(NSData *data, NSError *error) {
        [self log:[NSString stringWithFormat:@"%@",data]];
    }];
//    [self log:[NSString stringWithFormat:@"%@",data]];
