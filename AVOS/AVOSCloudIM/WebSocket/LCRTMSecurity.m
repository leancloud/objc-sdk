...
#import "LCRTMSecurity.h"

@interface LCRTMSSLCert ()
...
- (instancetype)initUsingPublicKeys:(BOOL)publicKeys {
    ...
    NSMutableArray<LCRTMSSLCert*> *collect = [NSMutableArray array];
    for(NSString *path in paths) {
        NSData *data = [NSData dataWithContentsOfFile:path];
        if(data) {
            [collect addObject:[[LCRTMSSLCert alloc] initWithData:data]];
        }
    }
    return [self initWithCerts:collect publicKeys:publicKeys];
}
/////////////////////////////////////////////////////////////////////////////
- (instancetype)initWithCerts:(NSArray<LCRTMSSLCert*>*)certs publicKeys:(BOOL)publicKeys {
    if(self = [super init]) {
        ...
        if(self.usePublicKeys) {
            dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT,0), ^{
                ...
                for(LCRTMSSLCert *cert in certs) {
